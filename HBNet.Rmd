...
En suivant le protocole de Zhou et al. (2022), nous vous créer une base de données d'interactions PPIs humain - bactérie/fungus, où les identifiants ne sont pas des protéines mais des clusters d'homologie UniRef, afin de propager les PPIs aux protéines proches structurellement. Les informations de cette base de données sont ensuite utilisées pour traiter les données métagénomiques de cohortes sur l'acné ou d'autres maladies cutanées.

## a. Listes de PPI venant de Zhou et al. (2022)

Voir **"Additional file 9: Table S8"** de la publication. Le fichier contient les IDs UniProt des PPIs inter-spécifiques, avant tout filtre. Les interactions proviennent de diverses grosses bases de données, ainsi que de publications (text mining manuel).
...
```{r statistic_species echo=FALSE}
...
En se basant sur la colonne "taxonomic lineage", on détermine le groupe d'où provient la protéine. Au final, on ne s'intéresse qu'aux protéines humaines, bactériennes et fongiques de notre système peau.
...
Notre table contient 130M de lignes (tout UniProt!). On veut filtrer pour avoir seulement les 8937 interacteurs qui nous intéressent. On se sert de l'utilitaire **sgrep** ([SourceForge](https://sourceforge.net/projects/sgrep/)) qui utilise la recherche dichotomique pour trouver les 8,937 lignes au sein des 141M.

Encore une fois, éditer le script pour le faire sur UniRef50/90/100.
...
On peut enlever les quelques accessions qui n'ont pas eu de matchs lors du premier ID mapping sur le site d'UniProt (seulement 29).

```{r remove_empty}
# Liste des IDs à enlever.
unique_interactors$From[is.na(unique_interactors$Organism)]

unique_interactors <- unique_interactors[!is.na(unique_interactors$Organism),]
```
