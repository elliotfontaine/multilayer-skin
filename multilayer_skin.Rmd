...
On exclue les entrées où il manque un ID UniProt (ceux ayant '-' comme valeur), pour éviter de se retrouver avec un vertex '-' ne correspondant pas à une protéine.
```{r biogrid_human}
...
Nos "terminals" dans le sens du problème de l'arbre de Steiner sont les nœuds correspondants aux protéines de l'environnement métabolique proche.
Comme on utilise une heuristique (SP = Shortest Path based approximation), le sous-réseau obtenu sera différent (en nombre de vertices et edges) à chaque fois qu'on exécute le code suivant, à moins d'utiliser une graine aléatoire fixée.
...
Rendu à ce point, les nœuds ont 3 attributs: **name** qui donne l'ID UniProt, **color** qui indique s'il s'agit des protéines ciblées ou des protéines rajoutées au graphe par `steinertree()`, et **type** qui a pour valeur "protein". Le **type** indique la couche au sein du réseau multicouche. Nous allons par la suite ajouter des noeuds de **type** "drug".

# **II. Couche des médicaments (drugs)**

## Creation du graphe isolé des médicaments
...
drugbank <- read_csv("drugbank_uniprot_links_2.csv") %>%
...
## Creation du "multiplex" avec `combine_layers`
