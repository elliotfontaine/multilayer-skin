...
Rendu a ce point, les nœuds ont 3 attributs: **name** qui donne l'ID UniProt, **color** qui indique s'il s'agit des protéines ciblées ou des protéines rajoutées au graphe par `steinertree()`, et **type** qui a pour valeur "protein". Le **type** indique la couche au sein du reseau multicouche. Nous allons par la suite ajouter des noeuds de **type** "drug".
...
# Infos sur les nœuds
vertex_names <- unique(c(drugbank$`DrugBank ID`, drugbank$`UniProt ID`))
drugbank_attributes <- data.frame(
  name = vertex_names,
  type = ifelse(grepl("^DB", vertex_names), "drug", "protein"),
  color = ifelse(grepl("^DB", vertex_names), "pink", "yellow")
)
remove(vertex_names)

# Création du graphe
graph_drugbank <- igraph::graph_from_data_frame(
  drugbank_relations,
  vertices = drugbank_attributes,
  directed = FALSE
)
summary(graph_drugbank)
print(graph_drugbank, e = T, v = T)
```
